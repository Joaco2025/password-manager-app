// src/renderer/src/components/EntryDetailModal.tsx
...
export const EntryDetailModal = ({ isOpen, onClose, entries, BrandIcon, onDelete }: Props) => {
  ...
  const handleDeleteClick = () => {
    setShowConfirmDelete(true) // Abrimos el modal de confirmación personalizado
  }

  const confirmDelete = () => {
    onDelete(currentEntry.id)
    showToast('Credential deleted permanently', 'info')
    setShowConfirmDelete(false)
    
    // Si quedan más entradas en el grupo, reseteamos al primero para evitar errores
    if (entries.length > 1) {
      setCurrentIndex(0)
    }
  }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        ...
        <div className="w-full max-w-md bg-[#0f111a] border border-white/10 rounded-2xl shadow-2xl overflow-hidden relative animate-in zoom-in-95 duration-300">
          ...
          <div className="px-6 py-6 bg-slate-900/50 border-b border-white/5 flex flex-col items-center relative">
            ...
            {/* Indicadores de Puntos (Solo si hay múltiples) */}
            {hasMultiple && (
